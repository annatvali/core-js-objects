...
function mergeObjects(objects) {
  return objects.reduce((mergedObject, obj) => {
    return {
      ...mergedObject,
      ...Object.fromEntries(
        Object.entries(obj).map(([key, objValue]) => [
          key,
          (mergedObject[key] || 0) + objValue,
        ])
      ),
    };
  }, {});
...
function removeProperties(obj, keys) {
  ...
  if (Array.isArray(keys)) {
    keys.forEach((key) => delete updatedObj[key]);
  } else {
    delete updatedObj[keys];
  }
...
function compareObjects(obj1, obj2) {
  const keysSource = Object.keys(obj1);
  const keysTarget = Object.keys(obj2);

  if (keysSource.length !== keysTarget.length) {
    return false;
  }

  return keysSource.every((key) => obj1[key] === obj2[key]);
}
...
function makeWord(lettersObject) {
  return Array.from(
    Object.entries(lettersObject).reduce((acc, [letter, indexes]) => {
...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    const valuesA = Object.values(a).join('');
    const valuesB = Object.values(b).join('');
...
function group(array, keySelector, valueSelector) {
  return new Map(
    [...new Set(array.map(keySelector))].map((key) => [
      key,
      array
...
class SelectorBuilder {
  ...
  addSelector(type, value) {
    ...
    if (
      this.orderOfSelectors
        .slice(this.orderOfSelectors.indexOf(type) + 1)
        .some((order) =>
